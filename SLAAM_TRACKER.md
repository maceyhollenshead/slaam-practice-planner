...
0. **Lock decisions:** confirm shared-login provisioning + `active` sub; **drill storage = new `drills` table mirroring `plays` (DECIDED 2026-06-16** — keeps the public play-library / `is_public` machinery uncoupled, gives drills real metadata columns, copy-adapts the proven `plays.py` RLS pattern); freeze Plan/Segment + Drill JSON from the prototype; stub nullable `org_id` on new tables (`team_id` already real).
...
**Brain forward-compat (2026-06-16) — does this schema let the future "brain" tag practices/drills, incl. uploaded ones?** Yes, *if* we add the seams now (cheap now, expensive to retrofit). Bake into the `drills` + `practice_plans` tables from day one:
- `tags TEXT[] DEFAULT '{}'` on **both** (plays already has this) → the brain can write structured labels (skill, phase, intensity, age) without a migration. `category`/`focus` cover human tags; `tags` + `ai_labels` cover machine tags.
- `source TEXT DEFAULT 'manual'` (`manual | upload | ai_generated`) + `source_file TEXT NULL` (Supabase Storage path) → **the landing zone for uploaded practices.** A coach uploads a PDF/photo of a practice → row stored with `source='upload'`, raw file in `source_file`; the brain later extracts `segments`/drills and writes them back to the same row. Same pattern the prototype already uses for per-segment attachments, lifted to the record level.
- `ai_labels JSONB DEFAULT '{}'` (a.k.a. brain metadata) → embeddings refs, classifier outputs, taxonomy, confidence — kept separate from human-entered fields so the brain never clobbers coach input.
- Because every row already carries `team_id` (+ stub `org_id`), the brain can learn **per-program** (the IP-capture goal) and stay tenant-scoped. Net: tagging + ingestion are **additive columns now**, not a redesign later. (Actual upload→extract pipeline = a later phase; we're only reserving the columns.)

**Process note:** `frontend/CLAUDE.md` mandates plan-first + **never push `main`** (Vercel/Railway auto-deploy from it); work on `dev`/feature branch. The reusable engine + auth/DB mean the planner is a **module, not a rebuild** — new work ≈ 3 tables + 3 CRUD routes + porting the prototype UI.
